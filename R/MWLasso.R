...
CDM <- function (List, Y, Nk, cc, lambda, eta, d, epson = 1e-10, M = 100) {
  X_1 = List$X_1; index_1 = List$index_1;
  X_0 = List$X_0; index_0 = List$index_0;
  p = ncol(X_1); q = ncol(X_0);
  beta = rep(0,p);
...
pred_num <- function(List, Y, Nk, cc, d, lambda, eta, epson = 1e-10, M = 100) {
  eta = eta/(d-1);
...
find_lambda <- function(List, Y, Nk, cc, d, number, gamma2, epson = 1e-10, M = 100){
  X_1 = List$X_1; index_1 = List$index_1;
  p = ncol(X_1)
  max_lambda = rep(0, p)
  for ( i in 1:p ) {
...
CDM_qualitative <- function(List, Y, Nk, cc, initial.beta0, lambda, eta, d, epson = 1e-10, M = 100){
  X_1 = List$X_1; index_1 = List$index_1;
  X_0 = List$X_0; index_0 = List$index_0;
  X = X_1;
...
pred_num_qualitative <- function(List, Y, Nk, cc, initial.beta0, d, lambda, eta, epson = 1e-10, M = 100) {
  eta = eta/(d-1);
...
find_lambda_qualitative <- function(List, Y, Nk, cc, initial.beta0, d, number, gamma2, epson = 1e-10, M = 100){
  X_1 = List$X_1; index_1 = List$index_1;
  p = ncol(X_1);
  max_lambda = rep(0, p)
  for ( i in 1:p ) {
...
MWLasso <- function(X, Y, lambda, eta, d, method='linear', epson = 1e-10, M = 100){
  n = nrow(X)
  List = choose_X(X, n)
  List = Stand_X(List)
  Nk = get_Nk(List)
...
MW_parameters <- function(X, Y, d, number, gamma2, method="linear", epson = 1e-10, M = 100){
  n = nrow(X)
  List = choose_X(X, n)
  List = Stand_X(List)
  Nk = get_Nk(List)
